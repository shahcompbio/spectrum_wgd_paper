...
def create_medicc2_cell_list(
    ...
    # for MEDICC2, exclude multipolar cells
    my_cells = cell_info[(cell_info.include_cell) & 
                        (cell_info.patient_id == patient_id) & ~(cell_info.multipolar)].brief_cell_id
    my_cells.to_csv(output_filename, index=False, header=False)

if __name__ == "__main__":
    logging.basicConfig(stream=sys.stderr, level=logging.INFO)
    create_medicc2_cell_list()
