...
def construct_pp_tree(B):
    if np.max(np.sum(B, axis = 0)) < B.shape[0]:
        # add dummy clonal block to simplify this algorithm
        added_dummy = True
        B = np.concatenate([np.ones((B.shape[0], 1), dtype=int), B], axis = 1)
    else:
        added_dummy = False
    ...
    for i in range(len(col_groups)):
        if added_dummy:
            label = i - 1 if i > 0 else 'dummy'
        else:
            label = i
        col2labels[col_order[col_groups[i]]].append(label)
