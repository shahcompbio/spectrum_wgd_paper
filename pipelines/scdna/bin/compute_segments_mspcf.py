#!/home/myersm2/miniconda3/envs/misseg/bin/python
...
def filter_adata(signals_adata):
    ...
    signals_adata = signals_adata[:, 
                                  ~np.all(
                                      np.isnan(signals_adata.layers['B']),
                                      axis = 0)
                                 ].copy()
...
def preprocess_data(adata, chr2centro_filename, n_bins_trim_telomere = 2, centromere_buffer_size = 2.5e6,
                  skipY = True):
    """
    ...
    chr2centro = json.load(open(chr2centro_filename, 'r'))
...
def find_segments_mspcf(
    ...
    signals_adata = ad.read(signals_adata)
    ...
    cn_table = unstack_adata(signals_adata, layers = ['state', 'B', 'A'])
    resegmented = resegment(cn_table, segments, cn_cols = ['state', 'B', 'A'],
    ...
    resegmented.loc[resegmented['A'] > resegmented['B'], 'state_phase'] = 'A-Gained'
    resegmented.loc[resegmented['A'] < resegmented['B'], 'state_phase'] = 'B-Gained'
    resegmented.loc[resegmented['B'] == 0, 'state_phase'] = 'A-Hom'
    resegmented.loc[resegmented['A'] == 0, 'state_phase'] = 'B-Hom'
    resegmented.loc[resegmented['A'] == resegmented['B'], 'state_phase'] = 'Balanced'

    # reorder columns and drop segment_idx
    resegmented = resegmented[['cell_id', 'chr', 'start', 'end', 'state', 'B', 'A', 'state_phase']]

    resegmented.to_csv(output_file, index = False, compression={'method':'gzip', 'mtime':0, 'compresslevel':9})
...
@click.argument('chr2centro_filename')
...
def run_mspcf(
    ...
    adata = find_segments_mspcf(signals_adata_path, cell_list_path, chr2centro_filename, output_path,
