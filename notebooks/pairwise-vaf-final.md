...
```python
{colors[i]:c2snv3[colors[i]] for i in range(len(colors))}
```
...
```python
def plot_patient(p, fname=None, margin = 0.05, hist_ratio=0.2, ticks=[0, 0.5, 1], dpi = 200):
    blocks_adata = ad.read_h5ad(f'{project_dir}/tree_snv/inputs/{p}_general_clone_adata.h5')
    ...
    fig = plt.figure(figsize=(2 * (len(clones)-1), 2 * (len(clones)-1)), dpi=dpi)
    subfigs = fig.subfigures(nrows=len(clones)-1, ncols=len(clones)-1)
    
    if len(clones) == 2:
        subfigs = {(0,0): fig.subfigures(1,1)}
    ...
    if len(clones) == 2:
        fig.suptitle(p.replace('SPECTRUM-', ''), y=0.99, x = 1)
    else:
        fig.suptitle(p.replace('SPECTRUM-', ''), y=0.99)
...
```python
plot_patient('SPECTRUM-OV-025', '../../figures/final/pairwise_vaf_025.svg')
```

```python
plot_patient('SPECTRUM-OV-045', '../../figures/final/pairwise_vaf_045.svg')
```
...
with PdfPages('../../figures/final/pairwise-vafs.pdf') as pdf:
    for i, p in enumerate(sorted(all_adatas.keys())):
        ...
        if all_empty:
            ...
            plt.savefig(f'../../figures/final/patient-pairwise-vafs/vafs_{p}.png', bbox_inches='tight', pad_inches=0.3, dpi = 150)
            plt.close()
        if i > 6:
            pass
            #break
