...
adata_wgd_subclone = scgenome.tl.cluster_cells(
    adata_wgd[(adata_wgd.obs['n_wgd'] == 1), (adata_wgd.var['has_allele_cn']) & (adata_wgd.var['chr'].isin(['8']))],
...
# Sort on just chromosome '8' for aesthetics
adata_wgd_plot_sorted = scgenome.tl.sort_cells(adata_wgd_plot[:, (adata_wgd_plot.var['chr'].isin(['8', '9']))], layer_name='state')
...
# Sort on just chromosome '9' for aesthetics
adata_nwgd_plot_sorted = scgenome.tl.sort_cells(adata_nwgd_plot[:, (adata_nwgd_plot.var['chr'] == '9')], layer_name='state')
...
# Example cell with deletion and focal amp


```python

cell_id = 'SPECTRUM-OV-006_S1_INFRACOLIC_OMENTUM-128733A-R23-C14'
# cell_id = 'SPECTRUM-OV-006_S1_INFRACOLIC_OMENTUM-128733A-R15-C27'
...
scgenome.tl.get_obs_data(adata, cell_id).sort_values('copy', ascending=False).head(20)[['copy']]
...
# chromosomes = ['7', '8', '16', '19']
# scgenome.refgenome.set_genome_version('hg19', chromosomes=chromosomes, plot_chromosomes=chromosomes)

print(len(adata_wgd_plot[g['adata'].obs.index].obs.index))

for cell_id in adata_wgd_plot[g['adata'].obs.index].obs.query(f'n_wgd == 1').index[5:27]:
    brief_cell_id = adata.obs.loc[cell_id, 'brief_cell_id']
    longest_135_segment = noisy_cells.set_index('brief_cell_id').loc[brief_cell_id, 'longest_135_segment']
    average_135_bias = noisy_cells.set_index('brief_cell_id').loc[brief_cell_id, 'average_135_bias']
    ...
    ax.set_ylim((0, 10))
    ax.get_legend().remove()
    ax.set_title(f'{cell_id}, {average_135_bias:.2f}, {longest_135_segment:.2f}', fontsize=10, y=1.1)
    ax.grid(True)
    ...
    ax = axes[1]
    spectrumanalysis.plots.plot_allele_cn_profile(
        adata,
        cell_id,
        ax=ax
    )
