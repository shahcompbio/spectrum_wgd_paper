...
for cell_id, cluster_id in adata_wgd.obs[cluster_label].items():
    ...
    for layer in layers:
        ...
        cluster_cn = np.array(adata_clusters[cluster_id].layers[layer][0])

        # HACK
        assert np.mean(cluster_cn == 0) < 0.02, f"bins with cn=0 is {np.mean(cluster_cn == 0)}"
...
adata_wgd_subclone = scgenome.tl.cluster_cells(
    ...
    layer_name='copy', max_k=2)

fig = plt.figure(figsize=(4, 3))
g = scgenome.pl.plot_cell_cn_matrix_fig(
    adata_wgd_subclone,
    fig=fig,
    layer_name='state',# raw=True,
...
wgd_subclone_cell_ids = adata_wgd_subclone.obs[adata_wgd_subclone.obs['cluster_id'].isin(['0', '1'])].index
...
fig, axes = plt.subplots(
    nrows=4, ncols=2,
    height_ratios=[10, nwgd_cells / 2, wgd_cells * 3, wgd_cells * 3],
    width_ratios=[20, 1],
